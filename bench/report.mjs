...
const report = {
	...
	shaping: {
		...
		fixedTokens: budget.at(-1)?.data.fixedTokens, scale: budget.at(-1)?.data.scale,
		overBudgetReal: budget.filter((b) => b.data.actualInput > b.data.budget).length,
		worstEstimateError: Math.max(0, ...budget.filter((b) => b.data.actualInput).map((b) => Math.abs(b.data.actualInput - b.data.estimatedInput) / b.data.actualInput)).toFixed(2),
	},
	recalls: recalls.length, nativeCompactions: compactions.length,
	folds: entries.filter((e) => e.type === "custom" && e.customType === "scoby-budget" && e.data.event === "fold").length,
